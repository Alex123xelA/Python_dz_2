...
def create_pivot_table(df):
    ...
    index = input("Введите столбцы для строк: ").strip().split(",")
    columns = input("Введите столбцы для столбцов: ").strip()
    if columns:
        columns = columns.split(",")
    else:
        columns = None
    values = input("Введите столбцы для агрегации values: ").strip().split(",")
    aggfunc = input("Введите функцию агрегации (sum, mean, count и т.д.): ").strip()
    fill_value = input("Введите значение для заполнения пропусков: ").strip()
    fill_value = fill_value if fill_value else None
